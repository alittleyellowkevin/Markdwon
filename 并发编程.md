...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/1a9e5054-b2c2-47f1-9e54-56e3202ef721.png) style="zoom: 67%;" />
...
* 当 Thread-2 执行 synchronized(obj) 就会将 Monitor 的所有者 Owner 置为 Thread-2，Monitor 中只能有一个 Owner，**obj 对象的 Mark Word 指向 Monitor**，把**对象原有的 MarkWord 存入线程栈中的锁记录**中（轻量级锁部分详解）
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/0133b414-cb4e-4b1a-b3b6-1b4cf8433305.png) style="zoom:67%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/6fc1a9a0-d4f4-4fd8-8bee-f21db333b349.png) style="zoom: 67%;" />
...
* 自旋成功的情况：
      ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/e035ad33-e72e-4dc8-a8ad-0f96128c43c3.png) style="zoom: 80%;" />

* 自旋失败的情况：

  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/a4c23ae0-160e-4efd-a5ce-60abc23704ae.png) style="zoom:80%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/03774575-df94-4838-9357-d9c5fd4c046a.png) style="zoom: 67%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/e7063a76-80f0-4fa0-b27a-1ea40a4594da.png) style="zoom: 50%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/340f2529-56e1-4fb3-8034-4b0c401d26bf.png) style="zoom: 67%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/0f1ec37d-ba7f-442d-8fea-d2904ce00084.png) style="zoom:80%;" />
...
* 读屏障（lfence，Load Barrier）保证在该屏障之后的，对共享变量的读取，从主存刷新变量值，加载的是主存中最新数据
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/c80a2e83-3dfd-46c2-97b0-91211d3a743d.png) style="zoom:67%;" />
...
* 有序性的保证也只是保证了本线程内相关代码不被重排序
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/7d57b2c7-4fcb-48a3-9912-1073479e91c0.png) style="zoom:67%;" />
...
* 探测式清理：沿着开始位置向后探测清理过期数据，沿途中碰到未过期数据则将此数据 rehash 在 table 数组中的定位，重定位后的元素理论上更接近 `i = entry.key & (table.length - 1)`，让**数据的排列更紧凑**，会优化整个散列表查询性能
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/a93862e6-1743-4e00-8539-728c87dd8202.png) style="zoom:67%;" />

  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/724df50e-1a98-4faf-8781-4a645928ec85.png) style="zoom:67%;" />
...
* 如果 key 使用强引用：使用完 ThreadLocal ，threadLocal Ref 被回收，但是 threadLocalMap 的 Entry 强引用了 threadLocal，造成 threadLocal 无法被回收，无法完全避免内存泄漏
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/aa41844c-3b5b-4263-9e89-c8d3182e2cc6.png) style="zoom:67%;" />
...
* 如果 key 使用弱引用：使用完 ThreadLocal ，threadLocal Ref 被回收，ThreadLocalMap 只持有 ThreadLocal 的弱引用，所以threadlocal 也可以被回收，此时 Entry 中的 key = null。但没有手动删除这个 Entry 或者 CurrentThread 依然运行，依然存在强引用链，value 不会被回收，而这块 value 永远不会被访问到，也会导致 value 内存泄漏
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/1bab745c-f1a5-41e1-8315-0f3bebc0c6be.png) style="zoom:67%;" />
...
* 请求的共享资源被占用，AQS 用队列实现线程阻塞等待以及被唤醒时锁分配的机制，将暂时获取不到锁的线程加入到队列中
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/8f4cd8ee-73c6-4647-bd14-351abc9d0b32.png) style="zoom: 80%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/a6e14235-1aca-4a7f-bf4f-55d7e986ce9c.png) style="zoom:80%;" />
...
* 接下来进入 addWaiter 逻辑，构造 Node 队列（不是阻塞队列），前置条件是当前线程获取锁失败，说明有线程占用了锁
  ...
  * Node 的创建是懒惰的，其中第一个 Node 称为 **Dummy（哑元）或哨兵**，用来占位，并不关联线程
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/73eb7188-677e-4eaf-96b9-da5b2627f83a.png) style="zoom:80%;" />
...
* 补充情况：查询线程 A 查询数据时恰好缓存数据由于时间到期失效，或是第一次查询
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/41cd3d1b-b551-48da-ae77-fa80328142c3.png) style="zoom:80%;" />
...
* 获取读锁失败，进入 sync.doAcquireShared(1) 流程开始阻塞，首先也是调用 addWaiter 添加节点，不同之处在于节点被设置为 Node.SHARED 模式而非 Node.EXCLUSIVE 模式，注意此时 t2 仍处于活跃状态
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/64d40104-cc95-4d04-8f29-3fe59d92f8f2.png) style="zoom: 80%;" />
...
* 接下来 t2 调用 setHeadAndPropagate(node, 1)，它原本所在节点被置为头节点；还会检查下一个节点是否是 shared，如果是则调用 doReleaseShared() 将 head 的状态从 -1 改为 0 并唤醒下一个节点，这时 t3 在 doAcquireShared 内 parkAndCheckInterrupt() 处恢复运行，**唤醒连续的所有的共享节点**
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/5e2c0829-d29d-44bf-bb76-074096680ce3.png) style="zoom: 67%;" />
...
* t4 在 acquireQueued 中 parkAndCheckInterrupt 处恢复运行，再次 for (;;) 这次自己是头节点的临节点，并且没有其他节点竞争，tryAcquire(1) 成功，修改头结点，流程结束
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/737a1110-bb3e-46d3-a8ab-e1d81d8a60f2.png) style="zoom: 67%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/baca56de-896f-4b9d-abc4-207dfbaef37e.png) style="zoom: 80%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/e6c9800e-49c8-40aa-adc6-5b459219571a.png) style="zoom:80%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/d92a4951-a90c-4164-997a-70de7603dab2.png) style="zoom: 50%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/aa4ffd4c-01ce-491d-b3a5-9bed2160ad15.png) style="zoom: 67%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/586473a7-37f1-4781-9055-e09645f63155.png) alt="三次握手" style="zoom: 50%;" />

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/b3b74b12-5ed7-4212-b27b-574b6a6dc79a.png) alt="四次挥手" style="zoom: 67%;" />
...
* 常用 API：`public Socket accept()`，**阻塞等待**接收一个客户端的 Socket 管道连接请求，连接成功返回一个 Socket 对象
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/3dace276-2981-4829-ba52-0eda369ffea1.png) style="zoom:67%;" />
...
* 位置、限制、容量遵守以下不变式： **0 <= position <= limit <= capacity**
  ...
  ![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/6ded6968-77e9-4714-be2b-8b55093b406a.png) style="zoom:67%;" />
...
![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/13f674a3-a327-44cc-a807-bbe5571ba0ae.png) style="zoom: 50%;" />

![](https://gulinall-hkw.oss-cn-shenzhen.aliyuncs.com/fd87cc82-d00f-4fdd-8e69-d45e5ee99ed3.png) style="zoom:50%;" />
